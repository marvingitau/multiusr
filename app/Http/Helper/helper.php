...
if (! function_exists('web_setting')) {
    ...
    function web_setting($key=null) {

        $gl_setting = \App\Model\WebSetting::first();
        if($key !== null){
           return $gl_setting->$key;
        }
        return $gl_setting;
    }
}
if (! function_exists('web_setting_item')) {

    function web_setting_item(string $type,bool $status = true) {

        return \App\Model\WebSettingItem::where('type',$type)->whereStatus($status)->get();
    }
}

if(!function_exists('show_ad')) {
    function show_ad($size) {
        $ad = \App\Model\Advertisement::where('size', $size)->where('is_active',1)->inRandomOrder()->first();
        if (!empty($ad)) {
            if($size == 1){
                $maxwd = '300px';
                $maxhi = '250px';
            ...
            if ($ad->type == 1) {
                return '<a target="_blank" href="'.$ad->redirect_url.'" onclick="increaseAdView('.$ad->id.')"><img src="'.asset('assets/backend/image/advertisement/'.$ad->image).'" alt="" style="width:100%; max-width:'.$maxwd.';height: 100%;max-height: '.$maxhi.'"/></a>';
            }
            if($ad->type == 2) {
                return $ad->script;
            }
...
if (! function_exists('date_range_to_arr')){
    function date_range_to_arr($range){
        ...
        $range = explode(' - ',$range);
        $result[0] = array_key_exists(0,$range)?$range[0]:null;
        $result[1] = array_key_exists(1,$range)?$range[1]:null;
        return $result;
